...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav
        className={`bg-[#387B36] shadow-md hidden sm:block transition-all duration-300 sticky top-0 z-50 `}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            ...
            <div className="hidden md:flex items-center space-x-6">
              <NavItem href={routes.home} icon={Home}>
                HOME
              </NavItem>
              <NavItem href="/marketplace" icon={ShoppingCart}>
                MARKETPLACE
              </NavItem>
              <NavItem href={routes.category} icon={Book}>
                RESOURCES
              </NavItem>
              <NavItem href={routes.contactus} icon={Phone}>
                CONTACT
              </NavItem>
            </div>
      ...
      <nav
        className={`bg-[#387B36]  sm:hidden transition-all duration-300 sticky top-0 z-50 `}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <Link to="/">
      ...
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 bg-[#387B36] shadow-md z-50">
        <div className="flex justify-around py-2">
          ...
          <NavItem href={routes.profile} icon={User}>
            Profile
          </NavItem>
        </div>
      </nav>
